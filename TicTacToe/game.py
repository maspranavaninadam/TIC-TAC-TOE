...
def computergame():
    ...
    def msg(keys, button_states):
        ...
        win = tk.Label(computergame.root, text="Winner is " + val, font="Helvetica 36 bold italic", fg="red")
        win.place(relx=0.3, rely=0.9)
        game_exit = tk.Button(computergame.root, text="exit", font="Helvetica 18 bold italic", fg="black",
                              bg="red", command=computergame.root.destroy)
    ...
    def check_winner():
        ...
        if button_states[keys[0]] == button_states[keys[1]] == button_states[keys[2]] != "":
            ...
            for i in keys:
                ...
                win = tk.Label(computergame.root, text="That's a draw", font="Helvetica 36 bold italic", fg="red")
    ...
    computergame.root = tk.Tk()
    computergame.root.title("TIC TAC TOE Computer")

    print(computergame.root.winfo_exists())
    # game.multi_button.pressed = False

    canvas = tk.Canvas(computergame.root, height=height, width=width)
    canvas.pack()

    frame = tk.Frame(computergame.root, bg='black', bd=10, cursor="dot")
    ...
    computergame.root.minsize(600, 600)
    computergame.root.maxsize(800, 800)

    computergame.root.mainloop()
